...
module.exports = movieUtils = {
    ...
    uploadMovies: function(key, callback){
        movieUtils.searchForMovie('', '', 0, 'blah', function (err, result){
            var cellHeaders = ['Title', 'Format', 'Year'];
            var tsv = json2csv({ data: result, fields: cellHeaders, del: '\t' });
            var drive = google.drive({ version: 'v3', auth: key });
            drive.files.create({
                resource: {
                    name: 'Movie List',
                    mimeType: 'application/vnd.google-apps.spreadsheet',
                    parents: [ '0B-pnJbq6rAmLTnNWVEVQekdwX3c' ]
                },
                media: {
                    mimeType: 'text/tsv',
                    body: tsv
