...
module.exports = movieUtils = {
    ...
    getStats: function (userId, callback) {
        ...
        var blankStats = {yearCounts: {}, formatCounts: {}, formatsByYear: {}};
        utils.getCouchbaseDocument(docKey).then(function (result) {
            callback(result);
        }).catch(function (err) {
            if(err.code === 13) {
                utils.upsertCouchbaseDocument(docKey, blankStats ).then(function(){
                    callback(blankStats)
                })
            }
            else {
    ...
    getMovies: function (userId, callback) {
        var docKey = 'uid_' + userId + '_list';
        utils.getCouchbaseDocument(docKey).then(function (result) {
            ...
            if (error.code === 13) {
                utils.upsertCouchbaseDocument(docKey, {movieList: []}).then(function () {
                    callback([])
                })
            }
            else {
