...
module.exports = utils = {
    saveMovie: function (title, year, format, poster, userId, callback) {
        ...
        movieBucket.get('uid_' + userId + '_list', function (err, result) {
            ...
            if (filteredMovies.length > 0) {
                ...
                newMovie.seen.push({format: format, seen: parseInt(year), poster: poster});
            } else {
            ...
            movieBucket.upsert('uid' + userId + '_list', {movieList: movieList}, function (err, result) {
                ...
                utils.updateStats(format, year, userId);
                callback(true)
            });
        })
    },
    ...
    updateStats: function (format, year, userId, callback) {
        ...
        movieBucket.get('uid_' + userId + '_stats', function (err, result) {
            ...
            movieBucket.upsert('uid' + userId + '_stats', currentStats, function (err, result) {
                if (err) {
                    callback(err);
                }
                callback(true)
