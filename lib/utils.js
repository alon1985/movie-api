...
module.exports = utils = {
    ...
    getNewMovie: function (movieList, title, year, format, poster, releaseYear, plot) {
        var newMovie;
        var filteredMovies = _.remove(movieList, function (movie) {
            if(movie.releaseYear && movie.releaseYear === releaseYear) {
        ...
        if (filteredMovies.length > 0) {
            ...
            if(newMovie.releaseYear != releaseYear) {
                newMovie.releaseYear = releaseYear;
            }
        } else {
            newMovie = {title: title, seen: [{format: format, seen: parseInt(year)}], poster: poster, plot: plot, releaseYear: releaseYear};
        }
        return newMovie;
    },
