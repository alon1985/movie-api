...
module.exports = utils = {
    ...
    updateYearStats: function (counter, stats, year) {
    ...
    updateFormatsByYearStats: function (stats, year, format, counter) {
        if(stats.formatsByYear[year]) {
            if (Number.isInteger(stats.formatsByYear[year][format])) {
                stats.formatsByYear[year][format] = stats.formatsByYear[year][format] + counter;
            } else if (counter > 0) {
                stats.formatsByYear[year][format] = 1;
            }
        } else if (counter > 0) {
            stats.formatsByYear[year] = {};
            stats.formatsByYear[year][_VIDEO] = 0;
    ...
    updateFormatsByYear: function (counter, stats, format, year) {
        stats = utils.updateFormatsByYearStats(stats, year, format, counter);
    ...
    updateStats: function (method, currentStats, format, year) {
        ...
        currentStats.yearCounts = utils.updateYearStats(counter, currentStats, year);
    ...
    getNewMovie: function (movieList, title, year, format, poster, plot) {
        ...
        var filteredMovies = _.remove(movieList, function (movie) {
            if(movie.title === 'All the President\'s Men') {
                var x = 0;
            }
