...
module.exports = processor = {
    ...
    saveMovieSeen: async function(existingMovieId, title, year, format, poster, plot) {
        if(!existingMovieId || existingMovieId === 0) {
    ...
    saveMovieToWatchlist: async function(existingMovieId, title, releaseDate, poster, plot) {
        if(!existingMovieId || existingMovieId === 0) {
            existingMovieId  = await processor._saveMovie(title, poster, plot)
        }
        if(existingMovieId) {
            const results = await dbUtils.saveToWatchlist(existingMovieId, releaseDate);
            return {
                status: 'success',
                result: {
                    movieId: existingMovieId,
